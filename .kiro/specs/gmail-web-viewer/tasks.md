...
- [ ] 3. Implement the messages API blueprint (`web/api/messages.py`)
  ...
  - [ ] 3.2 Add search filter (`q`) — case-insensitive substring match on `subject`, `sender` JSON text, and `body`
    - _Requirements: 3.1_
  - [ ] 3.3 Add label filter (`label`) — exact match within the `labels` JSON array
    - _Requirements: 3.2_
  - [ ] 3.4 Add boolean filters (`is_read`, `is_outgoing`) — accept `"true"` / `"false"` strings
